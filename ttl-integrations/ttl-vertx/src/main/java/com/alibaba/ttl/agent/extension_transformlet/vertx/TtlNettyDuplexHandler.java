 ...
 * @date: 2021/2/11
 */
public class TtlNettyDuplexHandler extends ChannelDuplexHandler implements TtlWrapper<ChannelInboundHandler>, TtlEnhanced, TtlAttachments {
    ...
    private TtlNettyDuplexHandler(@NonNull ChannelDuplexHandler handler, boolean releaseTtlValueReferenceAfterRun) {
    ...
    /**
     * Factory method, wrap input {@link ChannelInboundHandler} to {@link TtlNettyDuplexHandler}.
     *
     ...
     * @throws IllegalStateException when input is {@link TtlNettyDuplexHandler} already.
     */
    @Nullable
    ...
    /**
     * Factory method, wrap input {@link Handler} to {@link TtlNettyDuplexHandler}.
     *
     * @param handler                          input {@link Handler}. if input is {@code null}, return {@code null}.
     * @param releaseTtlValueReferenceAfterRun release TTL value reference after run, avoid memory leak even if {@link TtlNettyDuplexHandler} is referred.
     ...
     * @param handler                          input {@link ChannelInboundHandler}. if input is {@code null}, return {@code null}.
     * @param releaseTtlValueReferenceAfterRun release TTL value reference after run, avoid memory leak even if {@link TtlNettyDuplexHandler} is referred.
     * @param idempotent                       is idempotent mode or not. if {@code true}, just return input {@link ChannelInboundHandler} when it's {@link TtlNettyDuplexHandler},
     ...
     * @return Wrapped {@link ChannelInboundHandler}
     * @throws IllegalStateException when input is {@link TtlNettyDuplexHandler} already and not idempotent.
     */
    @Nullable
    public static ChannelHandler get(@Nullable ChannelHandler handler, boolean releaseTtlValueReferenceAfterRun, boolean idempotent) {
        ...
        if (handler instanceof ChannelInboundHandler) {
            ...
            return new TtlNettyDuplexHandler((ChannelDuplexHandler) handler, releaseTtlValueReferenceAfterRun);
        } else {
            return handler;
        }
    }
    ...
    /**
     * wrap input {@link ChannelInboundHandler} Collection to {@link TtlNettyDuplexHandler} Collection.
     *
    ...
    public static List<ChannelHandler> gets(@Nullable Collection<? extends ChannelInboundHandler> tasks, boolean releaseTtlValueReferenceAfterRun, boolean idempotent) {
        ...
        for (ChannelHandler task : tasks) {
            copy.add(TtlNettyDuplexHandler.get(task, releaseTtlValueReferenceAfterRun, idempotent));
        }
        return copy;
    }

    /**
     * Unwrap {@link TtlNettyDuplexHandler} to the original/underneath one.
     * <p>
     * this method is {@code null}-safe, when input {@code Function} parameter is {@code null}, return {@code null};
     * if input {@code Function} parameter is not a {@link TtlNettyDuplexHandler} just return input {@code Function}.
     * <p>
    ...
    public static ChannelInboundHandler unwrap(@Nullable ChannelInboundHandler handler) {
        if (!(handler instanceof TtlNettyDuplexHandler)) {
            return handler;
        } else {
            return ((TtlNettyDuplexHandler) handler).getHandler();
        }
    }

    /**
     * Unwrap {@link TtlNettyDuplexHandler} to the original/underneath one for collection.
     * <p>
    ...
    public static List<ChannelInboundHandler> unwraps(@Nullable Collection<? extends ChannelInboundHandler> tasks) {
        ...
        for (ChannelInboundHandler task : tasks) {
            if (!(task instanceof TtlNettyDuplexHandler)) {
                copy.add(task);
            } else {
                copy.add(((TtlNettyDuplexHandler) task).getHandler());
            }
        }
        return copy;
    }
    ...
    public boolean equals(Object o) {
        ...
        TtlNettyDuplexHandler that = (TtlNettyDuplexHandler) o;
