 ...
 * @date: 2021/2/11
 */
public class TtlNettyHandler implements ChannelInboundHandler, TtlWrapper<ChannelInboundHandler>, TtlEnhanced, TtlAttachments {
    ...
    private TtlNettyHandler(@NonNull ChannelInboundHandler handler, boolean releaseTtlValueReferenceAfterRun) {
    ...
    /**
     * Factory method, wrap input {@link ChannelInboundHandler} to {@link TtlNettyHandler}.
     *
     ...
     * @throws IllegalStateException when input is {@link TtlNettyHandler} already.
     */
    @Nullable
    ...
    /**
     * Factory method, wrap input {@link Handler} to {@link TtlNettyHandler}.
     *
     * @param handler                          input {@link Handler}. if input is {@code null}, return {@code null}.
     * @param releaseTtlValueReferenceAfterRun release TTL value reference after run, avoid memory leak even if {@link TtlNettyHandler} is referred.
     ...
     * @param handler                          input {@link ChannelInboundHandler}. if input is {@code null}, return {@code null}.
     * @param releaseTtlValueReferenceAfterRun release TTL value reference after run, avoid memory leak even if {@link TtlNettyHandler} is referred.
     * @param idempotent                       is idempotent mode or not. if {@code true}, just return input {@link ChannelInboundHandler} when it's {@link TtlNettyHandler},
     ...
     * @return Wrapped {@link ChannelInboundHandler}
     * @throws IllegalStateException when input is {@link TtlNettyHandler} already and not idempotent.
     */
    @Nullable
    public static ChannelHandler get(@Nullable ChannelHandler handler, boolean releaseTtlValueReferenceAfterRun, boolean idempotent) {
        ...
        if (handler instanceof ChannelInboundHandler) {
            ...
            return new TtlNettyHandler((ChannelInboundHandler) handler, releaseTtlValueReferenceAfterRun);
        } else {
            return handler;
        }
    }
    ...
    /**
     * wrap input {@link ChannelInboundHandler} Collection to {@link TtlNettyHandler} Collection.
     *
    ...
    public static List<ChannelHandler> gets(@Nullable Collection<? extends ChannelInboundHandler> tasks, boolean releaseTtlValueReferenceAfterRun, boolean idempotent) {
        ...
        for (ChannelHandler task : tasks) {
            copy.add(TtlNettyHandler.get(task, releaseTtlValueReferenceAfterRun, idempotent));
        }
        return copy;
    }

    /**
     * Unwrap {@link TtlNettyHandler} to the original/underneath one.
     * <p>
     * this method is {@code null}-safe, when input {@code Function} parameter is {@code null}, return {@code null};
     * if input {@code Function} parameter is not a {@link TtlNettyHandler} just return input {@code Function}.
     * <p>
    ...
    public static ChannelInboundHandler unwrap(@Nullable ChannelInboundHandler handler) {
        if (!(handler instanceof TtlNettyHandler)) {
            return handler;
        } else {
            return ((TtlNettyHandler) handler).getHandler();
        }
    }

    /**
     * Unwrap {@link TtlNettyHandler} to the original/underneath one for collection.
     * <p>
    ...
    public static List<ChannelInboundHandler> unwraps(@Nullable Collection<? extends ChannelInboundHandler> tasks) {
        ...
        for (ChannelInboundHandler task : tasks) {
            if (!(task instanceof TtlNettyHandler)) {
                copy.add(task);
            } else {
                copy.add(((TtlNettyHandler) task).getHandler());
            }
        }
        return copy;
    }
    ...
    public boolean equals(Object o) {
        ...
        TtlNettyHandler that = (TtlNettyHandler) o;
